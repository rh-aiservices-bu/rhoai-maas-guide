...
NOTE: The verification script deploys a temporary simulator model and removes it during the cleanup phase. Resources that already existed before the run - such as a simulator you deployed in xref:05-maas-models.adoc[Phase 5] - are detected and kept. Use `--no-cleanup` to keep even the script's own test resources, and `--cleanup-only` to remove all test resources including a pre-existing simulator.
