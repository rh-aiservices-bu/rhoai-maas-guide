...
[NOTE]
====
On RHOAI 3.5+, the `maas-api` deployment runs in `redhat-ai-gateway-infra`, and the `maas-db-config` secret must also exist in that namespace. The namespace is only created by the RHOAI operator in Phase 4, so pre-create it now (this is safe - the operator adopts it later) and create the mirrored secret. Use the fully-qualified service hostname `postgres.redhat-ods-applications.svc:5432` in the `DB_CONNECTION_URL` instead of the short name `postgres:5432` (which won't resolve from a different namespace):
...
oc create secret generic maas-db-config \
  ...
  --from-literal=DB_CONNECTION_URL="postgresql://maas:${POSTGRES_PASSWORD}@postgres.redhat-ods-applications.svc:5432/maas?sslmode=disable"
----

The `setup-maas.sh` script handles this mirroring automatically.
====
