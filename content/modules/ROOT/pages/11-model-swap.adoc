...
| MaaSModelRef name
| *No*
| This is the model identity that customers see. Keeping it the same is what makes the swap seamless.
...
Deploy a new LLMInferenceService alongside the existing one. Use the template in `manifests/99-model-swap/replacement-model/` or create your own.
...
vi manifests/99-model-swap/replacement-model/llm/model.yaml

# Deploy the replacement model
oc apply -k manifests/99-model-swap/replacement-model/
----
...
This one-field change is what makes the swap seamless. The MaaSModelRef name stays the same, so all auth policies, subscriptions, and API keys continue to work.
...
Test inference using the same API key and model ID as before the swap:
...
The `manifests/99-model-swap/` directory contains a template for the replacement model:
...
The template uses a CPU simulator by default. For a real swap, replace the container image, model URI, and resource requests in `model.yaml` to match the new model.

No `maas/` subdirectory is included because the MaaS control plane resources (MaaSModelRef, MaaSAuthPolicy, MaaSSubscription) already exist and do not change during a swap. Only the LLMInferenceService backend is replaced.
