...
[NOTE]
====
In {rhoai} 3.5, User Workload Monitoring is *no longer required* for MaaS. The RHOAI operator creates its own ServiceMonitors with `monitoring.opendatahub.io/scrape` labels, and the COO MonitoringStack (configured in Phase 7) handles metrics collection. UWM is still harmless to enable and useful if you want standalone Prometheus access to Kuadrant metrics outside of the RHOAI-managed monitoring stack.
====
