...
OpenAI gpt-oss-20b running on the Red Hat AI Inference Server. Uses the link:https://huggingface.co/RedHatAI/gpt-oss-20b[Red Hat AI validated quantized build^], which fits on a single 24 GB GPU (L4) or better.

* *Runtime*: vLLM CUDA (registry.redhat.io/rhaii/vllm-cuda-rhel9:3.3.0)
* *Model weights*: OCI modelcar from registry.redhat.io (~8GB download)
* *Resources*: 2-4 CPU, 16Gi-60Gi memory, 1x NVIDIA GPU
* *GPU memory*: ~16GB required (quantized - verified on a 24 GB L4)
* *Startup time*: 5-15 minutes (image pull + model loading)
...
NOTE: This model downloads weights from HuggingFace (`hf://`), so it is subject to the same HuggingFace Xet download hang as the simulator (see <<xet-hang,HuggingFace Xet Download Hang>> below). If the pod is stuck in `Init:0/1`, apply the `HF_HUB_DISABLE_XET` workaround.
...
Each model deployment creates the following custom resources. `LLMInferenceService` is a KServe resource (`serving.kserve.io/v1alpha1`); the MaaS resources belong to `maas.opendatahub.io/v1alpha1`:
