...
* *Multi-Tenancy Reference* - What RHOAI 3.5 MaaS multi-tenancy covers: tenants vs instances, dedicated vs shared resources, isolation boundaries, AITenant provisioning walkthrough, and quick answers for the common questions. (link:{repo-url}/pull/170[#170])
...
* *Demos Page* - 5 presenter-ready demos for subscription tiering, priority resolution, OIDC authentication, service account access, and JWKS cache validation. Contributed by deewhyweb. Grouped by parent phase with inline TIP pointers from Phase 5 and Phase 9. (link:{repo-url}/pull/152[#152], link:{repo-url}/pull/153[#153])

=== Features

* *Admin Key Management FAQ* - New section in the API Key Security page: why keys cannot be created on behalf of another user (and what works instead: self-service, IdP tokens with external OIDC, ServiceAccount tokens), and why there is no per-user key cap (quota is per subscription, not per key). (link:{repo-url}/pull/172[#172])

* *Site Search* - Search box in the top navbar, powered by the official Antora Lunr extension. Client-side index built at build time, results grouped by page with section deep links and snippets. (link:{repo-url}/pull/171[#171])

* *Verify Guide Skill Documentation* - Added `/verify-guide` skill to the Claude Code guide page with usage, arguments, phases table, and description of all 6 verification phases. (link:{repo-url}/pull/166[#166])

* *Azure OpenAI External Model Provider* - Added Azure OpenAI as fifth external model provider with Kustomize manifests, setup/cleanup script support, and gpt-4.1-mini as default model. Uses the `azure-openai` BBR provider type. (link:{repo-url}/pull/168[#168])

* *Anthropic External Model Provider* - Added Anthropic (Claude Haiku 4.5) as fourth external model provider with Kustomize manifests, setup/cleanup script support, and RHOAIENG-93984 known issue documentation for /v1/messages token counting gap. (link:{repo-url}/pull/164[#164])
...
* *External Models Dashboard Documentation* - Added Technology Preview callout for the External models tab in the RHOAI Dashboard (AI hub > Models > Deployments). New section in Phase 8 with screenshot showing the model listing and provider details. (link:{repo-url}/pull/156[#156])

=== Fixes

* *Broken Official Docs Links* - The RHOAI 3.5 docs renamed the MaaS chapter chunk and retired two anchors, breaking five deep links in Phases 2, 7, and 8. All links now point at the correct per-version chunk and anchors, verified against the live docs. (link:{repo-url}/pull/170[#170])

* *Live Install Review Fixes* - Docs, script, and demo fixes from a full manual install review: Phase 3 namespace pre-create for 3.5+, Phase 9 Keycloak hostname patch, verify.sh now keeps pre-existing models at cleanup, demo subscription priorities raised above the generic model tiers, and gpt-oss-20b VRAM corrected to ~16 GiB (runs quantized on a 24 GB L4). (link:{repo-url}/pull/169[#169])

* *UWM No Longer Required in RHOAI 3.5* - Marked User Workload Monitoring as optional in Phase 2. RHOAI 3.5 uses COO's MonitoringStack for metrics collection. UWM is still useful for forwarding metrics to external tools. (link:{repo-url}/pull/167[#167])
...
* *GPU Driver DTK Kernel Check* - Made the kernel check the gate for GPU driver builds on disconnected clusters. Driver builds only needed when DTK image kernel does not match the node. Contributed by deewhyweb. (link:{repo-url}/pull/69[#69])
...
| deewhyweb
| MaaS entitlement and authentication demos, GPU driver DTK kernel check
