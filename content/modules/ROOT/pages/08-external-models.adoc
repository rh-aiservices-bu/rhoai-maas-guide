...
WARNING: This phase is NOT compatible with disconnected/air-gapped environments. External models require internet access to reach third-party APIs (OpenAI, Bedrock, Gemini, Anthropic). If you are running in disconnected mode, skip this phase entirely.
...
The `--external-model-provider` flag selects which provider manifests to deploy (`openai`, `gemini`, `bedrock`, or `anthropic`; default: `openai`).
