...
NOTE: The RHOAI Dashboard also provides a *Technology Preview* UI for browsing external models under *Gen AI studio > AI asset endpoints*. The CLI workflow documented below (Steps 1-3) is the fully supported (GA) method. See <<dashboard-ui>> for the Dashboard UI walkthrough.
...
Both local models (LLMInferenceService) and external models (ExternalModel) appear in this table. Models registered via CLI (`oc apply`) and models registered via the Dashboard both show up here.
...
NOTE: Whether you manage external models via the Dashboard or via CLI, the same Kubernetes CRDs are created (`ExternalProvider`, `ExternalModel`, `MaaSModelRef`, `MaaSAuthPolicy`, `MaaSSubscription`). Models created via CLI appear in the Dashboard, and models visible in the Dashboard can be inspected with `oc get`.
