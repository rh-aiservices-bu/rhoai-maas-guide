...
NOTE: In {rhoai} 3.5, the monitoring stack uses the Cluster Observability Operator (COO) and its MonitoringStack CR for metrics collection. User Workload Monitoring (UWM) is no longer required. The RHOAI operator creates its own ServiceMonitors with `monitoring.opendatahub.io/scrape` labels that the COO-managed Prometheus scrapes.

This phase installs the *optional* observability operators and configuration:
...
For complete production setup steps, see the link:https://docs.redhat.com/en/documentation/red_hat_connectivity_link/1.4/html/install_connectivity_link/rhcl-install-on-ocp#configure-redis_install-on-ocp[Red Hat Connectivity Link - Configure Redis] documentation.
...
* link:https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.5/html/govern_llm_access_with_models-as-a-service/deploy-and-manage-models-as-a-service#maas-observability_maas-deploy[RHOAI 3.5 Observability dashboard]
* link:https://docs.redhat.com/en/documentation/red_hat_openshift_ai_self-managed/3.4/html/govern_llm_access_with_models-as-a-service/deploy-and-manage-models-as-a-service_maas#maas-observability_maas-deploy[RHOAI 3.4 Observability dashboard]
...
* link:https://docs.redhat.com/en/documentation/red_hat_connectivity_link/1.4/html/install_connectivity_link/rhcl-install-on-ocp#configure-redis_install-on-ocp[RHCL - Configure Redis for Limitador]
