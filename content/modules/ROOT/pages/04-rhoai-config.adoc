...
+
--
In RHOAI 3.5+, the maas-controller auto-creates a cluster-scoped `Config` CR (replacing the namespaced Tenant CR from 3.4):

[source,bash]
----
oc get configs.maas.opendatahub.io
----
--
...
* `externalModels: true` - enables the *External models* tab under *AI hub > Models > Deployments* in the Dashboard, where administrators can view deployed external models, their providers, and connection details

IMPORTANT: The *External models* tab is a *Technology Preview* feature in RHOAI 3.5. Technology Preview features are not supported with Red Hat production service level agreements (SLAs) and might not be functionally complete. The external models functionality itself (CRDs, CLI, gateway routing) is fully supported (GA). For CLI-based external model management, see xref:08-external-models.adoc[Phase 8: External Models].
